    ...
    <form id="login" action="<?php echo get_option('home'); ?>/wp-login.php" method="post" novalidate="novalidate">
     <div id="register-active" class="switch"><?php if(get_option('users_can_register')==1){ ?><i class="fa fa-toggle-on"></i>切换注册<?php } ?></div>       <h3>登录</h3>
        ...
        <div class="other-sign">
	      <p>您也可以使用第三方帐号快捷登录</p>
	        <div><a class="qqlogin" href="/wp-content/plugins/foxlogin/auth/qq.php?foxloginurl=<?php echo get_permalink($post->ID); ?>"><i class="fa fa-qq"></i><span>Q Q 登 录</span></a></div>
	  	  	  <div><a class="weibologin" href="/wp-content/plugins/foxlogin/auth/sina.php?foxloginurl=<?php echo get_permalink($post->ID); ?>"><i class="fa fa-weibo"></i><span>微 博 登 录</span></a></div>
	      </div>
	    </div> <?php  } ?>
